...
namespace CatLib.Routing
{
    ...
    public sealed class Router : IRouter
    {
        ...
        /// 当出现异常时的过滤器链
        /// </summary>
        private IFilterChain<IRequest, IResponse, System.Exception> onError;
        ...
        public Router(IEventDispatcher dispatcher, IContainer container)
        {
        ...
        {
            GuardExt.NotEmptyOrNull(uris, "uris");
        ...
        public IRoute Reg(string uris, Type controller, string method)
        {
            ...
            GuardExt.NotEmptyOrNull(method, "method");
            lock (syncRoot)
            {
                return RegisterRoute(uris, new RouteAction
                {
        ...
        /// <param name="priority">优先级(值越小越优先)</param>
        /// <returns>当前实例</returns>
        public IRouter OnError(Action<IRequest, IResponse, System.Exception, Action<IRequest, IResponse, System.Exception>> onError, int priority = int.MaxValue)
        {
            ...
            lock (syncRoot)
            {
                ...
                {
                    this.onError = new FilterChain<IRequest, IResponse, System.Exception>();
                }
        ...
        public void RouterCompiler()
        {
            lock (syncRoot)
            {
                ...
                if (compiler != null)
                {
                    if (dispatcher != null)
                    {
                        //Core 2.0.0版本 Trigger已经改成Raise了
                        dispatcher.Raise(RouterEvents.OnBeforeRouterAttrCompiler, this);
                    }
                    compiler.Complie();
                }
            }
        ...
        private bool ThrowException(Route route, IRequest request, IResponse response, System.Exception ex)
        {
        ...
        /// <param name="ex">异常</param>
        /// <returns>冒泡是否已经被拦截</returns>
        private bool ThrowOnError(IRequest request, IResponse response, System.Exception ex)
        {
        ...
        private IResponse RunRouteWithMiddleware(Route route, Request request, Response response)
        {
            if (dispatcher != null)
            {
                dispatcher.Raise(RouterEvents.OnDispatcher, new DispatchEventArgs(this, route, request));
            }
        ...
        {
            GuardExt.NotEmptyOrNull(uri, "uri");
